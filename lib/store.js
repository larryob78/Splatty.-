...
function list() {
  return loadProducts().map(p => ({
    ...
    points_url: p.points_url || null, splat_file: p.splat_file || null,
    gaussian_count: p.manifest.measured.gaussian_count || null
  }));
}
...
function search(query) {
  ...
  const results = loadProducts().map(p => {
    ...
    const dm = p.manifest.measured.dimensions_mm;
    const h = dm ? dm[2] : null;
    if (maxH !== null) { if (h !== null && h <= maxH) score += 2; else score = -1; }
    if (minH !== null) { if (h !== null && h >= minH) score += 2; else score = -1; }
...
async function searchSmart(query) {
  ...
  try {
    ...
    const scored = prods.map((p, i) => {
      ...
      return {
        id: p.id, name: p.name, brand: p.brand, category: p.category,
        gtin: p.manifest.gtin,
        height_mm: p.manifest.measured.dimensions_mm ? p.manifest.measured.dimensions_mm[2] : null,
